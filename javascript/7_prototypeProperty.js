...
// 생성자 함수(Aniaml) <------------------------------------------> Animal 프로토 타입 객체
...
// 생성자 함수(Animal) <--> new 객체( tiger, lion) --> new 연산자가 생성자 함수를 이용해서 새로운 객체를 생성.
// 하지만, 이렇게 생성된 객체들(tiger, lion)의 원형(부모)은 "(Animal) 프로토 타입 객체".
// 프로토타입 체인이 어떻게 연결되는지를 잘 기억하는게 중요!!

// [4] : 인스턴스(new 객체)가 어떤 생성자 함수로 생성된 것인지를 알아내는 방법?
// 위에서 말한 --> "(Animal) 프로토타입 객체" 
//              + constructor <-- 이게 생성자 함수를 가리키고 있으므로 이 값을 확인하면 됨.

// new 연산자로 새롭게 만들어진 인스턴스 (객체)는 자신의 부모격인 원형(프로토타입 객체)에서 특성(속성, 메서드)을 상속받으므로,
// 당연히 "(Animal) 프로토타입 객체의" constructor 속성을 참조 가능.
...
// instanceof 연산자 --> 객체와 생성자 2개가 필요 --> 이항 연산자.
...
testObj2.__proto__ = new A();

console.log(testObj instanceof A);
console.log(testObj2 instanceof A);
...
// 헷갈림 주의
// 1) __proto__ 으로나 prototype 이나 둘다 선언하여 속성 접근 가능
